...
async function autoLookupAndBind(userId, displayName) {
  try {
    ...
    let res = await fetch('http://yidianyuan.ao-lan.cn/wepapi/Customer/SearchCustomer', {
      method: 'POST',
      headers: headers,
      body: JSON.stringify([{ Key: 'KeyWord', Value: userId }])
    });
  ...
  if (target) return extractCustomerNo(target.CustomerNumber);
    ...
    res = await fetch('http://yidianyuan.ao-lan.cn/wepapi/Customer/SearchCustomer', {
      method: 'POST',
      headers: headers,
      body: JSON.stringify([{ Key: 'KeyWord', Value: displayName }])
    });
    data = await res.json();
...
setInterval(async () => {
  ...
  try {
    ...
    const searchData = await searchRes.json();
    const orders = searchData?.Data?.Data ?? [];
    console.log(`[AutoProgress] 第一筆訂單範例: ${JSON.stringify(orders[0]).substring(0, 300)}`);
...
app.post('/api/photo-delete', async (req, res) => {
  try {
    const { customerNumber, fileName, photoUrl } = req.body;
    const cloudinary = require('cloudinary').v2;
