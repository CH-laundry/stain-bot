...
// ============ 🆕 取件追蹤 API ============
app.post('/api/pickup/update-status', async (req, res) => {
  const { action, receivingItemId, deliverOrderId, status, timestamp } = req.body;
  
  logger.logToFile(`[取件追蹤] 收到更新請求: ${receivingItemId} - ${status}`);
  
  if (action !== 'updatePickupStatus') {
    return res.json({ success: false, error: '無效的 action' });
  }
  ...
  try {
    ...
    if (fs.existsSync(PICKUP_FILE)) {
      const fileContent = fs.readFileSync(PICKUP_FILE, 'utf8');
      data = JSON.parse(fileContent);
    }
    
    const order = data.orders.find(o => 
      o.receivingItemId === receivingItemId || 
      o.deliverOrderId === deliverOrderId ||
      (o.customerNumber && receivingItemId && receivingItemId.includes(o.customerNumber))
    );
    
    if (order) {
      order.pickedUp = true;
      order.signedTime = timestamp || new Date().toISOString();
      order.status = 'completed';
      order.nextReminderAt = null;
      
      fs.writeFileSync(PICKUP_FILE, JSON.stringify(data, null, 2), 'utf8');
      
      logger.logToFile(`✅ [取件追蹤] 已更新為已簽收: ${order.customerNumber} - ${order.customerName}`);
      ...
      logger.logToFile(`⚠️ [取件追蹤] 找不到訂單: ${receivingItemId}`);
      res.json({ 
        success: false, 
        error: '找不到訂單',
    ...
    logger.logError('[取件追蹤] 更新失敗', error);
    res.status(500).json({ 
      success: false, 
      error: error.message 
    });
