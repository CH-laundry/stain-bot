...
【🧧 春節店休公告 - 2026/2/14(六) ~ 2/21(六)】

**生效時間：2026 年 2 月 14 日至 2 月 21 日**
...
console.log('✅ 非表情符號，繼續處理');
    ...
    // 🧧 春節店休攔截（2/14 ~ 2/21，直接回覆不經過 AI）
const cnyNow = new Date(now.toLocaleString('en-US', { timeZone: 'Asia/Taipei' }));
const cnyMonth = cnyNow.getMonth() + 1;
const cnyDay = cnyNow.getDate();
const isChineseNewYear = (cnyMonth === 2 && cnyDay >= 14 && cnyDay <= 21);

if (isChineseNewYear) {
  console.log(`🧧 春節店休期間 ${cnyMonth}/${cnyDay}`);

  let cnyReply = '';

  // 1️⃣ 詢問何時開始營業
  if (/(什麼時候開|幾號開|何時營業|什麼時候恢復|幾號恢復)/.test(userMessage)) {
    cnyReply = `我們將於 2/22(日) 年後正常營業 💙\n到時會立即為您服務，謝謝您`;

  // 2️⃣ 緊急需求
  } else if (/(很急|緊急|急件|來不及)/.test(userMessage)) {
    cnyReply = `了解您的需求 💙\n\n🧧 2/14(六)-2/21(六) 新春店休\n年後 2/22(日) 開始我們會優先為您處理\n\n非常感謝您的體諒`;

  // 3️⃣ 打招呼/問候
  } else if (/(你好|在嗎|新年快樂|過年好|恭喜發財|新春快樂)/.test(userMessage)) {
    cnyReply = `新年快樂！🧧\n\n2/14(六)-2/21(六) 新春店休\n年後 2/22(日) 開始正常營業\n\n提前祝您新年快樂，萬事如意 💙`;

  // 4️⃣ 收件/送回/營業相關
  } else if (/(來收|收件|收衣|可以收|能收|送回|送到家|可以送|能送|有營業|有開|開門|營業時間|幾點開|過年|年假|店休|休息|有在|可以來|能來)/.test(userMessage)) {
    cnyReply = `🧧 2/14(六)-2/21(六) 新春店休\n\n年後 2/22(日) 開始正常營業 💙\n屆時我們會立即為您服務，謝謝您`;

  // 5️⃣ 一般洗衣問題（價格/項目等）→ 交給 AI 正常回答，但結尾加店休提醒
  } else {
    console.log('🧧 春節期間一般問題，交給 AI 回答');
    // 不設定 cnyReply，讓程式繼續往下跑交給 AI
  }

  // 如果有設定春節回覆，直接回傳不經過 AI
  if (cnyReply) {
    console.log('🧧 春節直接回覆:', cnyReply);
...
if (finalReply.includes('UNRELATED')) {
  ...
  return null;
    ...
    // 🧧 春節期間一般問題，結尾加店休提醒
if (isChineseNewYear && finalReply && !finalReply.includes('UNRELATED')) {
