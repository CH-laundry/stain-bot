...
function quickReply(userMessage) {
  ...
  const unrelatedPatterns = [
    /^(你好|hi|hello|嗨|在嗎|哈囉)$/i,
    /天氣|氣象|溫度/,
    /美食|餐廳|好吃/,
    /數學|計算|[0-9]+[\+\-\*\/][0-9]+/,
    /^(謝謝|感謝|thanks|thank you)$/i
  ];
  ...
  // 3. 簡單詢問是否有服務（不問價格）
  if (/^.{0,15}(可以|能|有)?.{0,5}洗.{0,10}(嗎|ㄇ|\?|？)$/.test(userMessage)) {
    if (/羽絨/.test(userMessage)) return '有的 💙 我們有清洗羽絨衣的服務';
    if (/外套|大衣|夾克/.test(userMessage)) return '有的 💙 我們有清洗外套的服務';
    if (/襯衫|上衣|t.?shirt|polo/i.test(userMessage)) return '有的 💙 我們有清洗襯衫上衣的服務';
    if (/褲子|長褲|短褲/.test(userMessage)) return '有的 💙 我們有清洗褲子的服務';
    if (/裙子|洋裝/.test(userMessage)) return '有的 💙 我們有清洗裙子的服務';
    if (/棉被|被子/.test(userMessage)) {
      const replies = [
    ...
    if (/手推車|嬰兒車|娃娃車|推車/.test(userMessage)) return '有的 💙 我們有清洗寶寶手推車的服務';
    if (/汽座|安全座椅/.test(userMessage)) return '有的 💙 我們有清洗汽座的服務';
  }
  
  // 4. 包包相關（需區分是否問價格）
  if (/包|包包/.test(userMessage)) {
    const askingPrice = /(多少|價格|價錢|費用|收費)/.test(userMessage);
    const luxuryBrands = /lv|gucci|chanel|hermès|hermes|prada|dior|celine|fendi|burberry|coach|mk|longchamp/i.test(userMessage);
    
    if (luxuryBrands && askingPrice) {
      return '這邊會由專人跟您回覆，謝謝您 💙';
    }
    
    if (luxuryBrands && !askingPrice) {
      return '有的 💙 精品包我們有專業清洗服務，會依材質和狀況細心處理';
    }
  ...
  // 5. 鞋子相關
  if (/(鞋|球鞋|運動鞋|皮鞋)/.test(userMessage) && !/(多少|價格|價錢|費用)/.test(userMessage)) {
    const replies = [
  ...
  // 6. 進度查詢
  if (/(洗好|完成|進度|可以拿|能拿|取件)/.test(userMessage)) {
    return '您可以線上查詢 C.H精緻洗衣 🔍\nhttps://liff.line.me/2004612704-JnzA1qN6#/home\n或是營業時間會有專人回覆您，謝謝 🙏';
  }
...
// 精簡版知識庫（大幅減少 token）
// ====================================
const LAUNDRY_KNOWLEDGE_COMPACT = `你是 C.H 精緻洗衣客服。簡潔親切回答，適度用💙。

【核心規則】
1. 只答洗衣相關，其他答 UNRELATED
2. 不提供電話、不問地址
3. 不主動報價格/完工時間，除非客人明確問
4. 汙漬問題必說「不保證完全去除」

【價格表】(僅在客人問「多少錢/價格/費用」時才報價)
- 襯衫/T恤/POLO:88 女上衣:90 針織:110 毛衣:150
- 短褲:90 長褲/西裝褲:120 短裙:130 長裙:160 短洋裝:230 長洋裝:270
- 夾克/外套:200 厚外套:300 大衣:320 長大衣:380 羽絨衣:330 長羽絨:400
- 西裝兩截:230 毛料西裝:300
- 單人床單/被套:170 單人被:250 厚單人被:360 雙人床單/被套:220 雙人被:420 厚雙人被:550
- 運動鞋:300/350(麂皮) 高價鞋(5千以上):400/450 精品鞋(1萬以上):800/1000
- 包包:600-1000 精品包:由專人評估報價
- 寶寶手推車:1200 汽座:900
- 地毯:依尺寸(60x90cm:800 / 180x270cm標準客廳:2000 / 300x400cm:5000)

【精品額外費用】(客人問價格+提到品牌時說明)
Canada Goose/Moncler/Burberry/Gucci/Prada/Dior/Chanel等:
- 上衣褲裙精品:基本價+150
- 大衣外套精品:基本價+250

範例：加拿大鵝羽絨衣？→「羽絨衣清洗:NT$330元，精品衣物特別處理+NT$250元，總計NT$580元💙」

【精品包處理】
問「能洗嗎」→「有的💙精品包我們有專業清洗服務，會依材質和狀況細心處理」
問「多少錢」→「這邊會由專人跟您回覆，謝謝您💙」

【收件判斷】(你要根據當前時間自動判斷)
今天收件：
- 今天=週六→「因週六固定公休，明天會去收回的💙」
- 板橋+下午6點前→「好的💙」
- 其他→「好的💙明天會去收」

明天收件：
- 明天=週六→「明天是週六固定公休，我們週日會去收回的💙」
- 其他→「好的💙明天會去收」
...
【特殊問題】
Q:外套內層可拆？A:可拆以2件計價
Q:汙漬/髒污/油漬能洗掉？A:好！我們會針對汙漬加強處理💙 ⚠️重要提醒：汙漬處理【不保證能完全去除】若汙漬已深入纖維或時間過久，可能無法完全處理
Q:完工時間？A:完工時間約7-10個工作日
Q:地毯？A:(完整流程+價格表)
Q:手推車/汽座？A:(完整清洗內容+價格+含到府收送)

【營業資訊】
營業時間:每日10:30-20:00(週六公休)
付款:現金/轉帳/LINE PAY/ECPay
完工:7-10工作日

【回覆風格】
簡潔、親切、專業、不重複、適度emoji💙🧺✨👔👟👜`;

// ====================================
// 手推車/汽座完整回覆
// ====================================
const BABY_ITEMS_REPLY = `💙 有的！我們有清洗寶寶手推車和汽座的服務
...
// 地毯完整回覆
// ====================================
const CARPET_REPLY = `💙 我們有專業地毯清洗服務
...
async function handleTextMessage(userMessage, userId = null) {
  try {
    ...
    if (quickAnswer) {
      ...
      // 特殊完整回覆
      if (/地毯/.test(userMessage) && /(多少|價格|價錢|費用|清洗)/.test(userMessage)) {
        return CARPET_REPLY;
      }
      
      if (/(手推車|嬰兒車|娃娃車|推車|汽座|安全座椅)/.test(userMessage) && /(多少|價格|價錢|費用)/.test(userMessage)) {
    ...
    const message = await anthropic.messages.create({
      model: "claude-haiku-4-5-20251001", // ✅ 改用 Haiku（省80%）
      max_tokens: 400, // ✅ 限制輸出長度
      system: LAUNDRY_KNOWLEDGE_COMPACT, // ✅ 精簡知識庫（省90%）
      messages: [{
        role: "user",
...
### 1. **三層防護，大幅省錢**
```
第一層：快速回覆（免費） → 處理 70% 簡單問題
第二層：避免重複（免費） → 防止收件問題重複回應
第三層：Claude Haiku（省錢） → 只處理複雜問題
```
...
| 項目 | 原版 | 優化版 | 省錢比例 |
|------|------|--------|---------|
| 簡單問題 (70%) | $0.012/次 | $0 (快速回覆) | 省 100% |
| 複雜問題 (30%) | $0.012/次 | $0.0008/次 | 省 93% |
| **100次對話** | **$1.2** | **$0.024** | **省 98%** 🎉 |

### 4. **新增成本監控**
每次呼叫 Claude 會在 console 顯示：
```
[Claude] Input: 245 tokens, Output: 67 tokens, Cost: $0.000464
const OpenAI = require('openai');
