...
app.get('/oauth2callback', async (req, res) => {
  ...
  try {
    ...
    res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>授權成功</title><style>body{font-family:sans-serif;text-align:center;padding:50px;background:linear-gradient(135deg,#667eea,#764ba2);color:white}.container{background:rgba(255,255,255,.1);border-radius:20px;padding:40px;max-width:500px;margin:0 auto}h1{font-size:32px;margin-bottom:20px}</style></head><body><div class="container"><h1>✅ 授權成功!</h1><p>Google Sheets 和 Drive 已成功連接</p><p>您可以關閉此視窗了</p></div></body></html>`);
  } catch (error) {
...
app.get('/test-upload', (req, res) => {
  res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>測試照片上傳</title><style>body{font-family:sans-serif;max-width:800px;margin:50px auto;padding:20px}h1{color:#333}.upload-section{background:#f5f5f5;padding:20px;border-radius:10px;margin:20px 0}.upload-section h2{color:#667eea;margin-top:0}button{background:#667eea;color:#fff;padding:10px 20px;border:none;border-radius:5px;cursor:pointer;font-size:16px}button:hover{background:#5568d3}.result{margin-top:20px;padding:15px;border-radius:5px}.success{background:#d4edda;color:#155724}.error{background:#f8d7da;color:#721c24}</style></head><body><h1>📸 測試照片上傳到 Google Drive</h1><div class="upload-section"><h2>洗前照片</h2><form id="uploadFormBefore"><input type="file" id="imageFileBefore" accept="image/*" required><br><br><button type="submit">上傳洗前照片</button></form><div id="resultBefore" class="result"></div></div><script>document.getElementById('uploadFormBefore').addEventListener('submit',async(e)=>{e.preventDefault();const fileInput=document.getElementById('imageFileBefore');const resultDiv=document.getElementById('resultBefore');if(!fileInput.files[0]){resultDiv.innerHTML='<div class="error">請選擇照片!</div>';return}resultDiv.innerHTML='<div>⏳ 上傳中...</div>';const formData=new FormData();formData.append('image',fileInput.files[0]);formData.append('type','before');try{const response=await fetch('/api/test-upload-image',{method:'POST',body:formData});const data=await response.json();if(data.success){resultDiv.innerHTML=`<div class="success"><h3>✅ 上傳成功!</h3><p><a href="${data.viewLink}" target="_blank">查看照片</a></p></div>`;fileInput.value=''}else{resultDiv.innerHTML=`<div class="error">❌ 上傳失敗: ${data.error}</div>`}}catch(error){resultDiv.innerHTML=`<div class="error">❌ 錯誤: ${error.message}</div>`}});</script></body></html>`);
});
...
app.get('/payment/success', (req, res) => {
  res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>付款完成</title><style>body{font-family:sans-serif;text-align:center;padding:50px;background:linear-gradient(135deg,#667eea,#764ba2);color:white}h1{color:#fff;font-size:32px}p{font-size:18px}.container{background:rgba(255,255,255,.1);border-radius:20px;padding:40px;max-width:500px;margin:0 auto}</style></head><body><div class="container"><h1>✅ 付款已完成</h1><p>感謝您的支付,我們會盡快處理您的訂單</p><p>您可以關閉此頁面了</p></div></body></html>`);
});

// ============== LINE Pay 取消頁面 ==============
app.get('/payment/linepay/cancel', (req, res) => {
  res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>付款取消</title><style>body{font-family:sans-serif;text-align:center;padding:50px;background:linear-gradient(135deg,#f093fb,#f5576c);color:white}.container{background:rgba(255,255,255,.1);border-radius:20px;padding:40px;max-width:500px;margin:0 auto}</style></head><body><div class="container"><h1>❌ 付款已取消</h1><p>您已取消此次付款</p><p>如需協助請聯繫客服</p></div></body></html>`);
});

// ============== LINE Pay 持續付款頁面(168小時內可重新生成連結) ==============
app.get('/payment/linepay/pay/:orderId', async (req, res) => {
  ...
  if (!order) {
    return res
      .status(404)
      .send(
        `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>訂單不存在</title><style>body{font-family:sans-serif;text-align:center;padding:50px;background:linear-gradient(135deg,#f093fb,#f5576c);color:white}.container{background:rgba(255,255,255,.1);border-radius:20px;padding:40px;max-width:500px;margin:0 auto}</style></head><body><div class="container"><h1>❌ 訂單不存在</h1><p>找不到此訂單</p></div></body></html>`
      );
  }
  ...
  if (orderManager.isExpired(orderId)) {
    ...
    logger.logToFile(
      `❌ 訂單已過期: ${orderId} (已過 ${hoursPassed.toFixed(1)} 小時)`
    );

    return res.send(
      `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>訂單已過期</title><style>body{font-family:sans-serif;text-align:center;padding:50px;background:linear-gradient(135deg,#f093fb,#f5576c);color:white}.container{background:rgba(255,255,255,.1);border-radius:20px;padding:40px;max-width:500px;margin:0 auto}h1{font-size:28px;margin-bottom:20px}p{font-size:16px;margin:15px 0}</style></head><body><div class="container"><h1>⏰ 訂單已過期</h1><p>此訂單已超過 7 天(168 小時)</p><p>已過時間: ${Math.floor(
        hoursPassed
      )} 小時</p><p>訂單編號: ${orderId}</p><p>請聯繫 C.H 精緻洗衣客服重新取得訂單</p></div></body></html>`
    );
  }

  if (order.status === 'paid') {
    return res.send(
      `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>訂單已付款</title><style>body{font-family:sans-serif;text-align:center;padding:50px;background:linear-gradient(135deg,#667eea,#764ba2);color:white}.container{background:rgba(255,255,255,.1);border-radius:20px;padding:40px;max-width:500px;margin:0 auto}</style></head><body><div class="container"><h1>✅ 訂單已付款</h1><p>此訂單已完成付款</p><p>訂單編號: ${orderId}</p></div></body></html>`
    );
  }
  ...
  try {
    ...
    const linePayResult = await createLinePayPayment(
      order.userId,
      order.userName,
      order.amount
    );
    ...
    if (linePayResult.success) {
      orderManager.updatePaymentInfo(
        orderId,
        linePayResult.transactionId,
        linePayResult.paymentUrl
      );

      const remainingHours = Math.floor(
        (order.expiryTime - Date.now()) / (1000 * 60 * 60)
      );

      res.send(
        `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>前往付款</title><style>body{font-family:sans-serif;text-align:center;padding:50px;background:linear-gradient(135deg,#667eea,#764ba2);color:white}.container{background:rgba(255,255,255,.1);border-radius:20px;padding:40px;max-width:500px;margin:0 auto}h1{font-size:28px;margin-bottom:20px}p{font-size:16px;margin:15px 0}.btn{display:inline-block;padding:15px 40px;background:#fff;color:#667eea;text-decoration:none;border-radius:10px;font-weight:bold;margin-top:20px;font-size:18px}.info{background:rgba(255,255,255,.2);padding:15px;border-radius:10px;margin:20px 0}</style></head><body><div class="container"><h1>💳 前往 LINE Pay 付款</h1><div class="info"><p><strong>訂單編號:</strong> ${orderId}</p><p><strong>金額:</strong> NT$ ${order.amount.toLocaleString()}</p><p><strong>剩餘有效時間:</strong> ${remainingHours} 小時</p></div><p>⏰ 付款連結 20 分鐘內有效</p><p>若超過時間,請重新點擊原始連結即可再次取得新的付款頁面</p><a href="${linePayResult.paymentUrl}" class="btn">立即前往 LINE Pay 付款</a></div><script>setTimeout(function(){window.location.href='${linePayResult.paymentUrl}'},2000);</script></body></html>`
      );
    } else {
      res
        .status(500)
        .send(
          `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>生成失敗</title><style>body{font-family:sans-serif;text-align:center;padding:50px}</style></head><body><h1>❌ 付款連結生成失敗</h1><p>${linePayResult.error}</p></body></html>`
        );
    }
  } catch (error) {
...
app.get('/payment/linepay/confirm', async (req, res) => {
  ...
  if (order && orderManager.isExpired(orderId)) {
    ...
    return res.send(
      `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>訂單已過期</title><style>body{font-family:sans-serif;text-align:center;padding:50px;background:linear-gradient(135deg,#f093fb,#f5576c);color:white}.container{background:rgba(255,255,255,.1);border-radius:20px;padding:40px;max-width:500px;margin:0 auto}</style></head><body><div class="container"><h1>⏰ 訂單已過期</h1><p>此訂單已超過 7 天</p></div></body></html>`
    );
  }
  ...
  try {
    ...
    if (result.returnCode === '0000') {
      ...
      logger.logToFile(
        `✅ LINE Pay 付款成功: ${decodeURIComponent(userName)} - ${amount}元`
      );

      res.redirect('/payment/success');
    } else {
      logger.logToFile(
        `❌ LINE Pay 付款確認失敗: ${result.returnCode} - ${result.returnMessage}`
      );
      res.send(
        `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>付款失敗</title><style>body{font-family:sans-serif;text-align:center;padding:50px}h1{color:#e74c3c}</style></head><body><h1>❌ 付款失敗</h1><p>${result.returnMessage}</p><p>請聯繫客服處理</p></body></html>`
      );
    }
  } catch (error) {
...
app.post('/send-payment', async (req, res) => {
  ...
  logger.logToFile(
    `收到付款請求: userId=${userId}, userName=${userName}, amount=${amount}, type=${paymentType}, customMessage=${customMessage}`
  );
  ...
  if (!userId || !userName || !amount) {
    ...
    return res
      .status(400)
      .json({ error: '缺少必要參數', required: ['userId', 'userName', 'amount'] });
  }
  ...
  try {
    ...
    if (type === 'linepay' || type === 'both') {
      ...
      if (linePayResult.success) {
        const order = orderManager.createOrder(linePayResult.orderId, {
          userId: userId,
          userName: userName,
          amount: numAmount,
        });
        ...
        const persistentUrl = `${
          process.env.RAILWAY_PUBLIC_DOMAIN ||
          'https://stain-bot-production-0fac.up.railway.app'
        }/payment/linepay/pay/${linePayResult.orderId}`;
    ...
    logger.logToFile(
      `✅ 已發送付款連結: ${userName} - ${numAmount}元 (${type})`
    );
...
app.listen(PORT, async () => {
  ...
  // 每日清一次過期訂單（orderManager 內部判斷 168 小時）
  setInterval(() => {
