...
// ★ 只註冊一次（你原本在 /send-payment 裡面 use 會每次呼叫重複掛），功能不變
const debugStorageRouter = require('./services/debugStorage');
...
// ★ 移到外面統一掛（保留功能）
app.use('/debug', debugStorageRouter);
...
// ====== ★★ 核心修正：建立 LINE Pay 付款（強制 capture、回傳 app/web 兩種連結、完整日誌）★★ ======
async function createLinePayPayment(userId, userName, amount) {
  try {
    ...
    if (result.returnCode === '0000') {
      ...
      logger.logToFile(`✅ paymentUrl.app: ${paymentUrlApp}`);
      logger.logToFile(`✅ paymentUrl.web: ${paymentUrlWeb}`);

      return {
        success: true,
        orderId,
        transactionId: result.info.transactionId,
        // ★ 預設回傳「app 優先」的可用連結（用於立即跳轉）
...
app.get('/test-sheets', async (req, res) => {
  try {
    ...
    await sheets.spreadsheets.values.append({
      ...
      resource: {
        values: [[timestamp, 'OAuth 測試客戶', 'test@example.com', '測試地址', 'OAuth 2.0 寫入測試成功! ✅']],
      },
    });
  ...
  try {
    ...
    if (type === 'linepay' || type === 'both') {
      ...
      if (linePayResult.success) {
        ...
        const paymentData = {
          linepayTransactionId: linePayResult.transactionId,
          linepayPaymentUrl: linePayResult.paymentUrl, // app 優先
        };
