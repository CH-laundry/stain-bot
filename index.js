...
// ==== 防止同一個 orderId 在短時間內重複呼叫 LINE Pay Request API ====
const inFlightLinePay = new Set();   // 記錄正在發起 LINE Pay 的 orderId
async function withLinePayLock(orderId, fn) {
  if (inFlightLinePay.has(orderId)) {
    // 已經有同一筆訂單在建立交易，避免重複建立多個 transactionId
    throw new Error('DUPLICATE_LINEPAY_REQUEST');
  }
  inFlightLinePay.add(orderId);
  try {
    return await fn();
  } finally {
...
// === 這支函式：加入 capture、詳細 log、同時回傳 app/web 並保留舊的 paymentUrl ===
async function createLinePayPayment(userId, userName, amount) {
    try {
        ...
        if (result.returnCode === '0000') {
            ...
            // 同時提供 app/web；並維持舊欄位 paymentUrl（優先 app）
            const paymentUrlApp = result.info.paymentUrl.app;
            const paymentUrlWeb = result.info.paymentUrl.web;
            const chosen = paymentUrlApp || paymentUrlWeb;
            return { 
                success: true, 
                paymentUrlApp, 
                paymentUrlWeb, 
                paymentUrl: chosen,           // 給舊程式使用
                orderId: orderId, 
...
// ===== LINE Pay：持久付款頁（防重複請求；優先 App Link；單次跳轉）=====
app.get('/payment/linepay/pay/:orderId', async (req, res) => {
  ...
  if (orderManager.isExpired(orderId)) {
    const hoursPassed = (Date.now() - order.createdAt) / (1000 * 60 * 60);
    logger.logToFile(`❌ 訂單已過期: ${orderId} (已過 ${hoursPassed.toFixed(1)} 小時)`);
    return res.send('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>訂單已過期</title><style>body{font-family:sans-serif;text-align:center;padding:50px;background:linear-gradient(135deg,#f093fb,#f5576c);color:white}.container{background:rgba(255,255,255,0.1);border-radius:20px;padding:40px;max-width:500px;margin:0 auto}h1{font-size:28px;margin-bottom:20px}p{font-size:16px;margin:15px 0}</style></head><body><div class="container"><h1>⏰ 訂單已過期</h1><p>此訂單已超過 7 天(168 小時)</p><p>訂單編號: ' + orderId + '</p></div></body></html>');
  }
  if (order.status === 'paid') {
    return res.send('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>訂單已付款</title><style>body{font-family:sans-serif;text-align:center;padding:50px;background:linear-gradient(135deg,#667eea,#764ba2);color:white}.container{background:rgba(255,255,255,0.1);border-radius:20px;padding:40px;max-width:500px;margin:0 auto}</style></head><body><div class="container"><h1>✅ 訂單已付款</h1><p>此訂單已完成付款</p><p>訂單編號: ' + orderId + '</p></div></body></html>');
  }
  ...
  try {
    // 若 20 分鐘內已有 reservation，就重用既有連結
    if (order.linepayTransactionId && order.linepayPaymentUrl) {
      const stillFresh = order.lastLinePayRequestAt && (Date.now() - order.lastLinePayRequestAt < 18 * 60 * 1000);
      if (stillFresh) {
        logger.logToFile(`↩️ 重用既有 LINE Pay 連結: ${orderId}`);
        const remainingHours = Math.floor((order.expiryTime - Date.now()) / (1000 * 60 * 60));
        return res.send(renderLinePayPage(orderId, order.amount, remainingHours, order.linepayPaymentUrl));
      }
    }

    // 沒有進行中交易 → 使用「防重複鎖」建立新交易
    ...
    if (!result.ok) {
      return res.status(500).send('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>生成失敗</title></head><body><h1>❌ 付款連結生成失敗</h1><p>請稍後重試</p></body></html>');
    }
    ...
    if (error && error.message === 'DUPLICATE_LINEPAY_REQUEST') {
      logger.logToFile(`⚠️ 阻擋重複 LINE Pay Request: ${orderId}`);
      return res.status(429).send('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>請稍候</title></head><body><h1>🔄 正在為您建立付款頁</h1><p>請勿重複點擊，稍候再試。</p></body></html>');
    }
...
<style>
body{font-family:sans-serif;text-align:center;padding:48px;background:linear-gradient(135deg,#667eea,#764ba2);color:white}
.container{background:rgba(255,255,255,0.12);border-radius:20px;padding:28px;max-width:560px;margin:0 auto}
.btn{display:inline-block;padding:14px 28px;background:#fff;color:#4e5bd5;text-decoration:none;border-radius:10px;font-weight:700;margin-top:16px;font-size:18px}
.pill{display:inline-block;padding:6px 12px;border:1px solid rgba(255,255,255,.5);border-radius:999px;margin:6px 4px}
.note{opacity:.9}
</style>
</head><body>
<div class="container">
  <h1>💳 前往 LINE Pay 付款</h1>
  <div class="pill">訂單編號：${orderId}</div>
  <div class="pill">金額：NT$ ${amount.toLocaleString()}</div>
  <div class="pill">剩餘有效時間：${remainingHours} 小時</div>
  <p class="note">⏰ 請在 <b>LINE App 內</b> 開啟此連結；若在外部瀏覽器，可能會被視為取消。</p>
  <a class="btn" href="${paymentUrl}">🔓 在 LINE App 內開啟並付款</a>
  <p class="note">若未自動跳轉，請點按上方按鈕。</p>
</div>
<script>
  // ✅ 僅做一次跳轉；不再做任何狀態輪詢或二次導向，避免中途被帶回而誤判取消
  (function jumpOnce(){
    try { window.location.href = "${paymentUrl}"; } catch(e){}
  })();
</script>
</body></html>`;
}
...
app.get('/payment/linepay/confirm', async (req, res) => {
    ...
    try {
        ...
        // ✅ 以訂單金額為準；若沒有訂單（理論上不會），再退回 query 的 amount
        const safeAmount = order ? parseInt(order.amount) : parseInt(amountFromQuery);
        const requestBody = { amount: safeAmount, currency: 'TWD' };
        const signature = generateLinePaySignature(uri, requestBody, nonce);
        ...
        if (result.returnCode === '0000') {
            ...
            if (userId && userId !== 'undefined') {
                await client.pushMessage(userId, { 
                    type: 'text', 
                    text: `✅ LINE Pay 付款成功\n\n感謝 ${decodeURIComponent(userName)} 的支付\n金額:NT$ ${safeAmount.toLocaleString()}\n訂單編號:${orderId}\n\n非常謝謝您\n感謝您的支持 💙` 
                });
            }
            
            logger.logToFile(`✅ LINE Pay 付款成功: ${decodeURIComponent(userName)} - ${safeAmount}元`);
    ...
    try {
        ...
        if (linePayResult.success) {
            const paymentData = {
                ...
                linepayPaymentUrl: linePayResult.paymentUrl // 仍相容舊欄位（優先 app）
            };
...
app.post('/send-payment', async (req, res) => {
    ...
    app.use('/debug', require('./services/debugStorage')); // ← 保留你原本的寫法
