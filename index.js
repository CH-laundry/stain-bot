...
/**
 * 產生 LINE Pay 付款；強制帶 options.payment.capture: true
 * 回傳同時包含 app/web 連結（並保留 paymentUrl 舊欄位）
 */
async function createLinePayPayment(userId, userName, amount) {
  try {
    ...
    if (result.returnCode === '0000') {
      logger.logToFile(`✅ paymentUrl.app: ${result.info.paymentUrl.app}`);
      logger.logToFile(`✅ paymentUrl.web: ${result.info.paymentUrl.web}`);
    } else {
      logger.logToFile(`❌ LINE Pay 付款請求失敗: ${result.returnCode} - ${result.returnMessage}`);
    }
    logger.logToFile(`📥 LINE Pay API 回應: ${JSON.stringify(result, null, 2)}`);

    if (result.returnCode === '0000') {
      logger.logToFile(`✅ LINE Pay 付款請求成功: ${orderId}`);
      return {
        success: true,
        paymentUrl: result.info.paymentUrl.web,     // 兼容舊欄位
        paymentUrlApp: result.info.paymentUrl.app,  // 新增：app link
        paymentUrlWeb: result.info.paymentUrl.web,  // 新增：web link
        orderId: orderId,
...
.container{background:rgba(255,255,255,0.1);border-radius:20px;padding:40px;max-width:500px;margin:0 auto}</style>
</head><body><div class="container"><h1>⚠️ ${title}</h1><p>${message}</p></div></body></html>`;
}

// ⛔ 取消自動跳轉，改成「手動按鈕」避免 LINE 判定可疑跳轉
...
<style>
body{font-family:sans-serif;text-align:center;padding:48px;background:linear-gradient(135deg,#06C755,#00B900);color:white;margin:0}
.container{background:rgba(255,255,255,0.15);border-radius:20px;padding:32px;max-width:560px;margin:0 auto;box-shadow:0 8px 32px rgba(0,0,0,0.1)}
h1{font-size:28px;margin-bottom:20px;font-weight:700}
.info{background:rgba(255,255,255,0.2);border-radius:12px;padding:16px;margin:20px 0}
.info-row{display:flex;justify-content:space-between;margin:8px 0;font-size:15px}
.info-label{opacity:0.9}
.info-value{font-weight:700}
.btn{display:block;width:100%;padding:18px;background:#fff;color:#06C755;text-decoration:none;border-radius:12px;font-weight:700;margin-top:24px;font-size:18px;border:none;cursor:pointer;transition:transform 0.2s}
.btn:active{transform:scale(0.98)}
.note{font-size:14px;opacity:0.9;margin-top:16px;line-height:1.6}
.warning{background:rgba(255,200,0,0.2);padding:12px;border-radius:8px;margin:16px 0;font-size:14px}
</style>
</head><body>
<div class="container">
  <h1>💳 LINE Pay 付款</h1>
  <div class="info">
    <div class="info-row"><span class="info-label">訂單編號</span><span class="info-value">${orderId}</span></div>
    <div class="info-row"><span class="info-label">付款金額</span><span class="info-value">NT$ ${Number(amount).toLocaleString()}</span></div>
    <div class="info-row"><span class="info-label">有效時間</span><span class="info-value">${remainingHours} 小時</span></div>
  </div>
  <div class="warning">⚠️ 請確保在 <b>LINE App 內</b> 開啟此頁面</div>
  <a href="${paymentUrl}" class="btn" onclick="handlePayment(event)">🔓 前往 LINE Pay 付款</a>
  <p class="note">點擊上方按鈕後請完成付款流程<br>付款成功後會自動通知我們<br><b>請勿重複點擊或關閉頁面</b></p>
</div>
<script>
let clicked=false;
function handlePayment(e){
  if(clicked){e.preventDefault();alert('付款處理中,請稍候...');return false;}
  clicked=true;
  const btn=e.target;btn.style.opacity='0.6';btn.textContent='⏳ 跳轉中,請稍候...';
  return true;
}
document.addEventListener('visibilitychange',function(){
  if(document.visibilityState==='visible' && clicked){
    setTimeout(function(){
      clicked=false;
      const btn=document.querySelector('.btn');
      if(btn){btn.style.opacity='1';btn.textContent='🔓 前往 LINE Pay 付款';}
    },3000);
  }
});
</script>
...
/* ---------------- LINE Pay：持久付款頁（重用＋鎖＋手動跳轉） ---------------- */

// 5 秒內，同一 IP 對同一訂單多次請求 → 擋重複
const recentPayPageHits = new Map();
function _hitKey(ip, orderId) { return `${ip}#${orderId}`; }
function isTooSoon(ip, orderId, ms = 5000) {
  const key = _hitKey(ip, orderId);
  const now = Date.now();
  const last = recentPayPageHits.get(key) || 0;
  recentPayPageHits.set(key, now);
  return (now - last) < ms;
}

// In-flight 鎖，避免並發重建 reservation
const inFlightLinePay = new Map();
async function withLinePayLock(orderId, fn) {
  if (inFlightLinePay.get(orderId)) {
    // 已有同訂單的重建流程在跑
    throw new Error('DUPLICATE_LINEPAY_REQUEST');
  }
  inFlightLinePay.set(orderId, true);
  try {
    return await fn();
  } finally {
    inFlightLinePay.delete(orderId);
  }
}

// ===== LINE Pay：持久付款頁（強制優先 app link；手動按鈕＋重用）=====
app.get('/payment/linepay/pay/:orderId', async (req, res) => {
  ...
  if (isTooSoon(ip, orderId)) {
    logger.logToFile(`🛡️ 5 秒內重複開啟付款頁：${orderId} from ${ip} → 嘗試重用現有連結`);
  ...
  // 20 分鐘內的 reservation → 直接重用
  if (order.linepayTransactionId && order.linepayPaymentUrl) {
    const lastRequest = order.lastLinePayRequestAt || 0;
  ...
  try {
    const result = await withLinePayLock(orderId, async () => {
      ...
      if (!lp.success) {
        return { ok: false, error: lp.error };
      }

      // ✅ 優先用 app link（若不存在則回退 web）
    ...
    if (!result.ok) {
      logger.logToFile(`❌ 建立 LINE Pay 交易失敗: ${result.error}`);
      return res.status(500).send(renderErrorPage('付款連結生成失敗', '請稍後重試或聯繫客服'));
    }

    const remainingHours = Math.floor((order.expiryTime - Date.now()) / (1000 * 60 * 60));
    return res.send(renderLinePayPage(orderId, order.amount, remainingHours, result.paymentUrl));

  } catch (error) {
    if (error && error.message === 'DUPLICATE_LINEPAY_REQUEST') {
      logger.logToFile(`⚠️ 阻擋重複請求: ${orderId}`);
      return res.status(429).send(renderErrorPage('請稍候', '付款頁面正在生成中，請勿重複點擊'));
    }
    logger.logError('LINE Pay 付款頁面錯誤', error);
    return res.status(500).send(renderErrorPage('系統錯誤', '請稍後重試'));
  }
});
...
/* ---------------- 訂單 API（維持原樣，僅在兩處補 lastLinePayRequestAt） ---------------- */
  ...
  try {
    ...
    if (linePayResult.success) {
      const paymentData = {
        ...
        linepayPaymentUrl: linePayResult.paymentUrlApp || linePayResult.paymentUrlWeb || linePayResult.paymentUrl,
        lastLinePayRequestAt: Date.now() // ✅ 補上
      };
...
app.post('/send-payment', async (req, res) => {
  ...
  app.use('/debug', require('./services/debugStorage')); // ← 保留你的原設定
    ...
    if (type === 'linepay' || type === 'both') {
      ...
      if (linePayResult.success) {
        ...
        const paymentData = {
          linepayTransactionId: linePayResult.transactionId,
          linepayPaymentUrl: linePayResult.paymentUrlApp || linePayResult.paymentUrlWeb || linePayResult.paymentUrl,
          lastLinePayRequestAt: Date.now() // ✅ 新增：初次建立就打上時間戳
        };
