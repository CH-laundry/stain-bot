...
try {
  if (process.env.GOOGLE_SHEETS_CREDENTIALS) {
    ...
    console.log('⚠️  Google Sheets 未設定（GOOGLE_SHEETS_CREDENTIALS）');
  }
} catch (error) {
...
2. 如果客戶問的問題完全無關，回覆：UNRELATED
3. **無關問題包括（必須回覆 UNRELATED）**：
   - 招呼語：「你好」「嗨」「hi」「hello」「在嗎」「您好嗎」「你好嗎」「早安」「晚安」
   - 感謝語：「謝謝」「感謝」「謝了」「thanks」「thank you」「3Q」
   - 閒聊：天氣、美食、數學題、新聞、八卦、笑話
   - 測試：「測試」「test」「1」「2」「3」
   - 其他完全與洗衣無關的內容
4. 完全不要提供電話號碼
...
9. **強制不回應清單**：以下文字已經有自動回應，你必須回覆 UNRELATED 不要再回應：
   ...
   - 「寶寶汽座&手推車」
   - 「顧客須知」
10. **對話脈絡判斷**：你會看到客人前幾則對話記錄，請根據對話脈絡精準判斷客人的意圖：
   - 如果客人在討論「送回」時間，不要誤判為「收件」
   - 如果客人說「好」「週一」等，要看前面對話判斷是在回應什麼
   - 「送到家」「送回」「完工」都是指送回，不是收件
   - 「約時間」「方便協調」通常是要約送回時間
   - 客人回覆時間（如「週一傍晚」），要看前文判斷是收件還是送回

【特殊情況處理 - 非常重要！必須使用禮貌模板】

🔴 情況 1：客人催件或抱怨太慢
觸發詞：「怎麼洗這麼久」「已經超過時間」「什麼時候好」「還沒好嗎」「洗很久」「這麼慢」「都幾天了」「超過很久」
...
🔴 情況 2：客人抱怨忘記收件
觸發詞：「忘記來收了」「還沒來收」「是不是忘了」「怎麼還沒來」「沒來收」「沒來拿」「忘記收衣服」
...
當天收件規則：
- 只有江子翠地址 + 詢問時間在下午 5 點前 → 回覆「好的 💙」
- 江子翠地址 + 詢問時間在下午 5 點後 → 回覆「好的 💙 明天會去收」
- 其他區域或情況 → 回覆「好的 💙 明天會去收」

客戶詢問收送時：
- 如果問「可以到府收送嗎」→ 回覆「可以的 💙」
- 如果說「衣物準備好了」「請來收」→ 回覆「好的 💙」
...
「好的 💙 完工後我們會提前聯絡您約送回時間」
「好的 💙 週一傍晚我們會幫您送回」
「好的 💙 我們會在完工前與您確認方便的送回時間」

錯誤回覆：
❌「好的我們會去收回的，謝謝您」（這是收件，不是送回！）
...
15. 【重要】根據對話脈絡判斷：看前幾則對話，判斷客人是在問收件還是送回
16. 【重要】區分收件和送回：
   - 收件：來收衣服、收件
   - 送回：送到家、送回、完工後、約時間
17. 【重要】遇到催件、忘記收件、客訴等情況，必須使用禮貌模板回覆
`;
...
function detectEmotion(message) {
  const angry = ['生氣', '很爛', '太差', '退費', '不滿意', '投訴', '抱怨'];
  const impatient = ['怎麼這麼久', '洗這麼久', '還沒好', '太慢', '很久', '都幾天了', '超過'];
  const complaint = ['忘記', '還沒來', '怎麼還沒', '是不是忘了', '沒來收', '沒來拿'];
...
function detectQuestionType(message) {
  ...
  if (/精品|名牌|LV|Gucci|Chanel/.test(message)) return '精品項目';
  return '其他';
}
...
async function handleTextMessage(userMessage, userId = null) {
  try {
    ...
    const greetingsAndThanks = [
      '你好', '您好', '嗨', 'hi', 'hello', '在嗎', '您好嗎', '你好嗎',
      '早安', '午安', '晚安', '晚上好', '早上好',
      '謝謝', '感謝', '謝了', 'thanks', 'thank you', '3q', '謝謝你', '謝謝您',
      '感恩', '多謝', 'thx',
      '測試', 'test', 'testing'
    ];
    
    // 檢查是否為純招呼語或感謝語（去除空白和標點符號後）
    const cleanMessage = userMessage.trim().toLowerCase()
      .replace(/[!！?？.,，。、\s]/g, '');
    
    if (greetingsAndThanks.some(phrase => cleanMessage === phrase || cleanMessage === phrase.toLowerCase())) {
      console.log('🔇 偵測到招呼語或感謝語，不回覆');
      return null;
    }
    ...
    console.log('📜 對話記錄數量:', history.length);
    
    // 呼叫 Claude API（使用 Haiku 4.5 節省成本）
    const message = await anthropic.messages.create({
      model: "claude-haiku-4-20250514", // 使用 Haiku 4.5（便宜 80%）
      max_tokens: 800, // 節省成本
